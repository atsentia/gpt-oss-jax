...
def create_notebook():
    ...
    md(
        ...
        "\n",
        "Analyze the Orbax checkpoint to verify FP8 conversion:\n",
        "- Shows dtype distribution in converted checkpoint\n",
        "- All BF16 parameters should now be FP8\n",
        "- Checkpoint size: ~21 GB (BF16) or ~11 GB (FP8)"
    )
    ...
    code(
        ...
        "    )\n",
        "    \n",
        "    # For FP8: Upcast to float32 for inference (JAX doesn't support FP8 ops yet)\n",
        "    if STRATEGY == \"fp8\":\n",
        "        print(\"Upcasting FP8 weights to float32 for inference...\")\n",
        "        params_inference = jax.tree_util.tree_map(\n",
        "            lambda x: x.astype(jnp.float32) if x.dtype == jnp.float8_e4m3fn else x,\n",
        "            params\n",
        "        )\n",
        "    else:\n",
        ...
        "        model=model, params=params_inference, prompt_tokens=prompt_tokens,\n",
