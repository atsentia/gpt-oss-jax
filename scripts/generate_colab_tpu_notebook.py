...
def create_notebook():
    ...
    # Cell 0: Local Mac Testing Setup (optional)
    md(
    ...
    code(
        ...
        "STRATEGY = \"mixed\"  # Mixed precision: BF16 (small params) + FP8 (experts)\n",
        "BF16_DTYPE = jnp.bfloat16\n",
        "FP8_DTYPE = jnp.float8_e4m3fn\n",
        "MEM_GB = 14  # Expected memory for mixed (1.8B BF16 × 2 + 10.1B FP8 × 1)\n",
        "\n",
        "# Set local paths (Mac uses current directory, not /content/)\n",
        "# ⚠️ UPDATE THIS: Point to the directory containing .safetensors files\n",
        "# Common paths:\n",
        "#   - \"gpt-oss-20b/original\" (if running from repo root)\n",
        "#   - \"/Users/yourname/models/gpt-oss-20b/original\" (absolute path)\n",
        "safetensors_path = Path(\"gpt-oss-20b/original\").resolve()  # ← UPDATE THIS PATH!\n",
        "orbax_path = str((Path(\"gpt-oss-20b-orbax-\" + STRATEGY).resolve()))\n",
        "\n",
    ...
    # Cell 2: Install Dependencies Header
    md(
    ...
    code(
        ...
        "    print(\"Downloading GPT-OSS-20B (13.8 GB)...\", flush=True)\n",
        "    \n",
        "    # Suppress progress bars to avoid Safari scrolling issues\n",
        "    import os\n",
        ...
        "    print(f\"✓ Downloaded: {safetensors_path}\", flush=True)\n",
        "else:\n",
        "    print(f\"✓ Using existing SafeTensors path: {safetensors_path}\")\n",
        "    print(\"  (Set in Cell 0 - skipping download)\")"
    )

    # Cell 7: SafeTensors Inspection Header
    md(
        "## 4a. Inspect SafeTensors Checkpoint\n",
        "\n",
        "Analyze the dtypes and sizes in the SafeTensors checkpoint:\n",
        "- Shows BF16 vs MXFP4 (uint8) distribution\n",
        "- Total parameters: ~12B (stored), ~21B (after MXFP4 decompression)\n",
        "- Checkpoint size: ~13.8 GB on disk"
    )

    # Cell 8: SafeTensors Inspection Code
    code(
        ...
        "\n",
        "print(\"Analyzing SafeTensors checkpoint...\", flush=True)\n",
        "print(\"=\" * 70)\n",
        "\n",
        ...
        "total_bytes = 0\n",
        "\n",
        "with safe_open(str(safetensors_path / 'model.safetensors'), framework='np', device='cpu') as f:\n",
        "    for key in f.keys():\n",
        ...
        "        bytes_per_elem = {'BF16': 2, 'U8': 1, 'F32': 4}.get(dtype, 4)\n",
        ...
        "\n",
        "print(f\"{'Dtype':<10} {'Tensors':<10} {'Parameters':<15} {'Size (GB)':<12} {'% Params':<10}\")\n",
        "print(\"-\" * 70)\n",
        "\n",
        ...
        "    print(f\"{dtype:<10} {count:<10} {params:>14,} {size_gb:>11.2f} {pct:>9.1f}%\")\n",
        "\n",
        "print(\"-\" * 70)\n",
        "print(f\"{'TOTAL':<10} {sum(s['count'] for s in dtype_stats.values()):<10} \"\n",
        "      f\"{total_params:>14,} {bytes_to_gb(total_bytes):>11.2f} {'100.0':>9}%\")\n",
        "\n",
        "print(\"\\nNote: MXFP4 (uint8) decompresses to ~20B parameters (84.9% of 21B model)\")"
    )

    # Cell 9: Convert to Orbax Header
    md(
        "## 4b. Convert SafeTensors to Orbax Format\n",
        "\n",
        ...
        "\n",
        "**Direct MXFP4→FP8 conversion:** MXFP4 weights decompress directly to FP8, skipping BF16 intermediate. This saves ~30GB RAM during conversion.\n",
        "\n",
        "Expected time: ~2-3 minutes (TPU runtime has 50GB+ RAM)"
    )
    ...
    code(
        ...
        "t0 = time.time()\n",
        "\n",
        "# For TPU v6e with mixed precision: Load on CPU, convert, then save\n",
        "# Mixed precision: BF16 (small params) + FP8 (expert weights)\n",
        "# This saves memory while preserving quality on important parameters\n",
        "with jax.default_device(jax.devices('cpu')[0]):\n",
        "    loader = WeightLoader(str(safetensors_path))\n",
        "    # Mixed precision loading:\n",
        "    # - BF16 params (15.1%) stay BF16: embedding, norms, attention, gates, biases\n",
        "    # - MXFP4 expert weights (84.9%) decompress to FP8: mlp1_weight, mlp2_weight\n",
        ...
        "    print(f\"After loading {STRATEGY.upper()}: {ram_after_load:.2f} GB (+{ram_after_load - ram_start:.2f} GB)\")\n",
        "    \n",
        "\n",
    ...
    # Cell 11: Orbax Inspection Header
    md(
        "## 4c. Inspect Orbax Checkpoint\n",
        "\n",
        "Analyze the Orbax checkpoint to verify mixed-precision conversion:\n",
        "- Shows dtype distribution in converted checkpoint\n",
        "- Mixed precision: BF16 (small params) + FP8 (expert weights)\n",
        "- Checkpoint size: ~12 GB (vs 21 GB BF16, vs 11 GB full FP8)\n",
        "\n",
        "**TPU v6e Memory Budget (32 GB HBM):**\n",
        "- Model weights: ~14 GB (3.6 GB BF16 + 10.1 GB FP8)\n",
        "- KV cache (4K ctx): ~0.3 GB\n",
        "- Activations: ~2-3 GB\n",
        "- **Total: ~17 GB** → **15 GB headroom** ✅"
    )

    # Cell 12: Orbax Inspection Code
    code(
        "import jax.numpy as jnp\n",
        "\n",
        "print(\"Analyzing Orbax checkpoint...\")\n",
        "print(\"=\" * 70)\n",
        "\n",
        "# Load checkpoint\n",
        "checkpointer = ocp.PyTreeCheckpointer()\n",
        "params_check = checkpointer.restore(orbax_path)\n",
        "\n",
        ...
        "            count_params(value, f\"{path}/{key}\" if path else key)\n",
        "    elif isinstance(tree, jnp.ndarray):\n",
        "        dtype = str(tree.dtype)\n",
        "        num_params = tree.size\n",
        "        num_bytes = tree.nbytes\n",
        "        \n",
        "        dtype_stats[dtype]['count'] += 1\n",
        "        dtype_stats[dtype]['params'] += num_params\n",
        "        dtype_stats[dtype]['bytes'] += num_bytes\n",
        "\n",
        "count_params(params_check)\n",
        "\n",
        "total_params = sum(s['params'] for s in dtype_stats.values())\n",
        "total_bytes = sum(s['bytes'] for s in dtype_stats.values())\n",
        "\n",
        "print(f\"{'Dtype':<10} {'Arrays':<10} {'Parameters':<15} {'Size (GB)':<12} {'% Params':<10}\")\n",
        "print(\"-\" * 70)\n",
        "\n",
        "for dtype in sorted(dtype_stats.keys()):\n",
        "    stats = dtype_stats[dtype]\n",
        ...
        "print(\"-\" * 70)\n",
        "print(f\"{'TOTAL':<10} {sum(s['count'] for s in dtype_stats.values()):<10} \"\n",
        "      f\"{total_params:>14,} {total_bytes / (1024**3):>11.2f} {'100.0':>9}%\")\n",
        "\n",
        "# Free memory\n",
        "del params_check\n",
        "import gc\n",
        "gc.collect()\n",
        "\n",
        "print(f\"\\n✓ Verification complete: {STRATEGY.upper()} checkpoint saved successfully\")"
    )

    # Cell 13: Load from Orbax Header
    md(
    ...
    md(
        ...
        "\n",
        "**Note for FP8:** JAX doesn't support FP8 operations yet, so FP8 weights are upcast to float32 during inference. This uses more memory temporarily but enables inference to work.\n",
        "\n",
    ...
    code(
        ...
        "    \n",
        "    # Note: FP8 weights are automatically upcast to float32 in generate()\n",
