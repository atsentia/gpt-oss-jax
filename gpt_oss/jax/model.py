...
# Import FlashAttention (optional optimization)
try:
    from .flash_attention import flash_attention, is_flash_attention_available
...
def sdpa(
    ...
    if optimize_gqa:
        # OPTIMIZED: Use broadcasting (no explicit memory copy)
        # Add singleton dimension for q_mult, einsum will broadcast automatically
        K = K[:, :, None, :]  # [n_kv_tokens, n_heads, 1, d_head]
        V = V[:, :, None, :]  # [n_kv_tokens, n_heads, 1, d_head]

        # Assert optimized shapes are correct for broadcasting
        assert K.shape == (n_kv_tokens, n_heads, 1, d_head), (
            f"[GQA-OPT] K shape mismatch after broadcast reshape:\n"
            f"  Expected: ({n_kv_tokens}, {n_heads}, 1, {d_head})\n"
            f"  Got: {K.shape}"
        )
        assert V.shape == (n_kv_tokens, n_heads, 1, d_head), (
            f"[GQA-OPT] V shape mismatch after broadcast reshape:\n"
            f"  Expected: ({n_kv_tokens}, {n_heads}, 1, {d_head})\n"
            f"  Got: {V.shape}"
        )
    else:
        # BASELINE: Explicit expansion (memory copy)
    ...
    # QK shape: [n_heads, q_mult, n_tokens, n_tokens]
    ...
    W = W[..., :-1]  # [n_heads, q_mult, n_tokens, n_tokens]

    # Apply attention weights to values
    attn = jnp.einsum('hmqk,khmd->qhmd', W, V)  # [n_new_tokens, n_heads, q_mult, d_head]
...
class AttentionBlock(nn.Module):
    ...
    Attributes:
        ...
        use_flash_attention: Whether to use FlashAttention (optional optimization)
    """
    config: ModelConfig
    layer_idx: int = 0
    ...
    def __call__(self, x: jax.Array, kv_cache: Optional[Any] = None) -> tuple[jax.Array, Optional[Any]]:
        ...
        # FlashAttention path: only when explicitly enabled AND flash backend available
        # Limitations: FlashAttention doesn't support sinks, so we use sdpa() with sinks=0 fallback
        if self.use_flash_attention and FLASH_ATTENTION_AVAILABLE:
            # FlashAttention wrapper expects:
            #   Q, K, V: [n_tokens, n_heads, d_head] (3D)
            # But we have:
            #   Q: [n_tokens, n_kv_heads, q_mult, d_head] (4D)
            #   K, V: [n_kv_tokens, n_kv_heads, d_head] (3D)

            # Reshape Q from 4D to 3D: [n_tokens, n_kv_heads, q_mult, d_head] -> [n_tokens, n_attention_heads, d_head]
            q_flash = q.reshape(n_tokens, num_attention_heads, head_dim)

            # Expand K/V for GQA: [n_kv_tokens, n_kv_heads, d_head] -> [n_kv_tokens, n_attention_heads, d_head]
            # FlashAttention requires same number of heads for Q, K, V
            k_flash = k[:, :, None, :].repeat(q_mult, axis=2).reshape(k.shape[0], num_attention_heads, head_dim)
            v_flash = v[:, :, None, :].repeat(q_mult, axis=2).reshape(v.shape[0], num_attention_heads, head_dim)

            # Call FlashAttention
            # Note: FlashAttention doesn't support sinks or sliding_window (backend-dependent)
            # For now, we ignore sinks and sliding_window when using FlashAttention
            # TODO Sprint 2.2: Add sink support via hybrid approach (FlashAttention + sink correction)
            attn_flash = flash_attention(
                q_flash, k_flash, v_flash,
                scale=sm_scale,
                causal=True,
                sliding_window=None  # Sliding window support is backend-dependent
            )

            # Reshape back to expected output format: [n_tokens, n_attention_heads, d_head] -> [n_tokens, n_attention_heads * d_head]
            attn_out = attn_flash.reshape(n_tokens, -1)

            # Validation
            expected_shape = (n_tokens, num_attention_heads * head_dim)
            assert attn_out.shape == expected_shape, \
                f"FlashAttention output shape mismatch: expected {expected_shape}, got {attn_out.shape}"
        else:
            # Standard SDPA path (supports sinks, sliding window, all features)
...
class MLPBlock(nn.Module):
    ...
    Attributes:
        config: Model configuration
        optimize_moe: If True, use token grouping optimization for better cache locality
    """
    config: ModelConfig
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        if self.optimize_moe:
            ...
            # BASELINE: Per-token processing (current implementation)
            ...
            # Apply MLP2
            mlp2_out = jnp.einsum('beck,bek->bec', selected_mlp2_weight, mlp1_out) + selected_mlp2_bias

            # Weighted sum of expert outputs
            expert_outputs = jnp.einsum('bec,be->bc', mlp2_out, expert_weights)
...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x: jax.Array, kv_cache: Optional[Any] = None) -> tuple[jax.Array, Optional[Any]]:
        ...
        attn = AttentionBlock(
            ...
            optimize_gqa=self.optimize_gqa,
            name='attn'
        )
        if kv_cache is not None:
        ...
        mlp = MLPBlock(config=self.config, optimize_moe=self.optimize_moe, name='mlp')
        x = mlp(x)
    ...
    def __call__(self, x: jax.Array, kv_caches: Optional[List[Any]] = None) -> tuple[jax.Array, Optional[List[Any]]]:
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            block = TransformerBlock(
                ...
                use_flash_attention=self.use_flash_attention,
                optimize_gqa=self.optimize_gqa,
                optimize_moe=self.optimize_moe,
